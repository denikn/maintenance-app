...
function actionsThatRequireCreate(action) {
    ...
    if (
        ...
        this.props.getCurrentUser().canUpdate(modelDef)
    ) {
        return true;
    }
    return false;
...
function actionsThatRequireDelete(action) {
    ...
    if (
        action !== 'delete' ||
        this.props.getCurrentUser().canDelete(modelDef)
    ) {
        return true;
    }
    return false;
...
function getConstantDisplayNameOrOld(modelType, fieldName, oldVal) {
    return modelsThatMapToOtherDisplayName[modelType] &&
        modelsThatMapToOtherDisplayName[modelType][fieldName]
...
class List extends Component {
    ...
    componentWillMount() {
        ...
        const translationStoreDisposable = translationStore.subscribe(
            translationState => {
                this.setState({
    ...
    renderFilters = () => {
        ...
        return (
            <div>
                ...
                />
                {getFilterFieldsForType(this.props.params.modelType).map(
                    filterField => {
                        const modelDefinition = this.context.d2.models[
                            this.props.params.modelType
                        ];
    ...
    render() {
        ...
        const paginationProps = {
            hasNextPage: () =>
                Boolean(this.state.pager.hasNextPage) &&
                this.state.pager.hasNextPage(),
            hasPreviousPage: () =>
                Boolean(this.state.pager.hasPreviousPage) &&
                this.state.pager.hasPreviousPage(),
            onNextPageClick: () => {
                this.setState({ isLoading: true });
        ...
        const availableActions = Object.keys(contextActions)
            ...
            .filter(actionName => {
                if (actionName === 'share') {
                    return (
                        this.context.d2.models[this.props.params.modelType] &&
                        this.context.d2.models[this.props.params.modelType]
                            .isShareable
                    );
                }
        ...
        const defaultReallyMeansNone = row => {
            if (
                ...
                row.categoryCombo___displayName ===
                    row.categoryCombo.displayName
            ) {
            ...
            <div>
                ...
                {!!this.state.sharing.model && (
                    <SharingDialog
                        ...
                        open={
                            this.state.sharing.model && this.state.sharing.open
                        }
                ...
                {!!this.state.translation.model && (
                    <TranslationDialog
                        ...
                        objectTypeToTranslate={
                            this.state.translation.model &&
                            this.state.translation.model.modelDefinition
                        }
                ...
                <CompulsoryDataElementOperandDialog
                    ...
                    dataElementOperands={
                        this.state.dataElementOperand.dataElementOperands
                    }
