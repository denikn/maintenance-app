...
contextActions.edit.subscribe(action => {
    goToRoute(
        [
            '/edit',
...
// TODO: The action assumes that the appState actually has state
contextActions.clone.subscribe(action => {
    goToRoute(
        [
            '/clone',
            appStore.state.sideBar.currentSection,
...
contextActions.delete.subscribe(({ data: model }) =>
    getD2().then(d2 => {
        snackActions.show({
            message: [
                ...
                model
                    ...
                    .then(() => {
                        ...
                        snackActions.show({
                            message: `${
                                model.displayName
                            } ${d2.i18n.getTranslation('was_deleted')}`
                        })
...
contextActions.compulsoryDataElements.subscribe(async ({ data: model }) => {
    const d2 = await getD2()
    const api = d2.Api.getApi()
    const getModelItem = () =>
        d2.models[model.modelDefinition.name].get(model.id, {
            fields: [
                ':all',
    ...
    const getDataElementOperands = () =>
        api
            .get('dataElementOperands', {
    ...
    const [modelItem, dataElementOperands] = await Promise.all([
        getModelItem(),
    ...
    const dataElementOperandsForDataSet = dataElementOperands
        .map(dataElementOperand =>
            Object.assign(dataElementOperand, {
...
contextActions.sectionForm.subscribe(action => {
    goToRoute(
        [
            '/edit',
            appStore.state.sideBar.currentSection,
...
contextActions.dataEntryForm.subscribe(action => {
    goToRoute(
        [
            '/edit',
            appStore.state.sideBar.currentSection,
...
contextActions.pdfDataSetForm.subscribe(({ data: model, complete, error }) => {
    getD2()
        .then(d2 => {
            window.open(
...
contextActions.runNow.subscribe(
    async ({ data: model, complete: actionComplete, error: actionFailed }) => {
        const d2 = await getD2()
        ...
        if (model.modelDefinition.name === 'predictor') {
            ...
            d2.Api.getApi()
                .post([model.modelDefinition.plural, model.id, 'run'].join('/'))
                .then(() => {
                    snackActions.show({
...
contextActions.preview.subscribe(
    ({ data: model, complete: actionComplete, error: actionFailed }) => {
        getD2()
            .then(d2 => {
                window.open(
