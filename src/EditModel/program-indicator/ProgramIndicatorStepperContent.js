...
const stepperConfig = () => {
    const editDetailsFields = difference(fieldOrder.for('programIndicator'), [
        'expression',
        'filter'
    ])
    ...
    const programIndicator$ = programIndicatorStore.map(get('programIndicator'))
    ...
    const stepComponents = {
        EditProgramIndicatorDetailsForm: connectEditForm(
            wrapInPaper(
                createFormFor(
                    programIndicator$,
                    'programIndicator',
...
const ProgramIndicatorStepperContent = compose(
