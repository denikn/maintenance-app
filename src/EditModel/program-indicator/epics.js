...
export const programIndicatorLoad = programIndicatorStore => action$ =>
    action$
...
const firstNotUndefinedIn = compose(
    first,
    filter(identity)
)
...
export const programIndicatorModelSaveResponses = action$ =>
    Observable.merge(
        action$
            .ofType(PROGRAM_INDICATOR_SAVE_SUCCESS)
            .mapTo(notifyUser('success')),
        action$.ofType(PROGRAM_INDICATOR_SAVE_ERROR).map(action => {
