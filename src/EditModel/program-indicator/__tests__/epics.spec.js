...
describe('Program indicator epics', () => {
    ...
    describe('programIndicatorLoad', () => {
        ...
        test('should load the requested programIndicstor by id from the api', done => {
            const action = {
            ...
            epic(createActionStreamFor(action)).subscribe(() => {
                const fieldFilters =
                    ':all,attributeValues[:all,attribute[id,name,displayName]],program[id,displayName,programType,programTrackedEntityAttributes[id,trackedEntityAttribute[id,displayName,valueType]]]'

                expect(mockD2.models.programIndicator.get).toBeCalledWith(
                    'pTo4uMt3xur',
                    { fields: fieldFilters }
                )
        ...
        test('should set a new instance of programIndicator onto the store without calling the api', done => {
            const action = {
                type: PROGRAM_INDICATOR_LOAD,
                payload: {
                    id: 'add'
