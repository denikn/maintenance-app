import {
    NOTIFICATION_STAGE_REMOVE,
    NOTIFICATION_STAGE_SAVE,
    NOTIFICATION_SET_ADD_MODEL,
    removeProgramNotificationSuccess,
    setEditModel,
    saveStageNotificationSuccess,
    saveStageNotificationError,
    NOTIFICATION_PROGRAM_SAVE,
    NOTIFICATION_PROGRAM_REMOVE,
} from './actions';
 ...
 */
const getProgramStageFromModel = (state, model) =>
    model.programStage && model.programStage.id
        ? getProgramStageById(state, model.programStage.id)
        : first(state.programStages);

const removeProgramStageNotification = action$ =>
    action$
        .ofType(NOTIFICATION_STAGE_REMOVE)
        .flatMap(({ payload: model }) =>
            Observable.of(model).flatMap(model =>
                eventProgramStore.take(1).map(eventProgramState => {
...
const saveProgramStageNotification = (action$, store) =>
    action$
        .ofType(NOTIFICATION_STAGE_SAVE)
        // FIXME: Remove href hack when d2 is fixed
        .do(({ payload: model }) => {
            model.dataValues.href = `${model.modelDefinition.apiEndpoint}/${
                model.id
            }`;
        })
...
const saveProgramNotification = (action$, store) =>
    action$
