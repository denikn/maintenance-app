...
import {
    PROGRAM_ATTRIBUTES_ADD,
    PROGRAM_ATTRIBUTES_REMOVE,
    PROGRAM_ATTRIBUTES_ADDREMOVE_COMPLETE,
    PROGRAM_ATTRIBUTES_EDIT,
    PROGRAM_ATTRIBUTES_EDIT_COMPLETE,
    PROGRAM_ATTRIBUTES_SET_ORDER,
} from './actions';
import snackActions from '../../../../Snackbar/snack.actions';

const programAttributeExistsInAttributeUidList = uids =>
    compose(includes(__, uids), get('trackedEntityAttribute.id'));
const keepProgramAttributesNotInUidList = uids =>
    filter(negate(programAttributeExistsInAttributeUidList(uids)));

const addAttributeToProgram = store => action$ =>
    action$
...
const removeAttributeFromProgram = store => action$ =>
    action$
        ...
        .map(action => {
            ...
            if (has('trackedEntityType.trackedEntityTypeAttributes', program)) {
                ...
                if (attributeIdsToRemove.length !== withoutTetAttrs.length) {
                    ...
                    snackActions.show({
                        message:
                            'cannot_remove_program_attribute_inherited_from_tet',
                        translate: true,
                    });
                }
            }
...
const editAttribute = store => action$ =>
    action$
...
const setAttributesOrder = store => action$ =>
    action$
