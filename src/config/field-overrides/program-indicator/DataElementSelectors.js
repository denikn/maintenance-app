...
const getAvailableDataElementsForProgram = memoize(program =>
    Observable.fromPromise(getInstance()).mergeMap(
        memoize(d2 =>
            d2.models.programStages
                .filter()
                .on('program.id')
                .equals(program)
                .list({
...
const DataElementSelectors = componentFromStream(props$ =>
    props$
        .flatMap(({ program, ...props }) =>
            getAvailableDataElementsForProgram(program).map(
                programStageCollection => {
                    const programStages = programStageCollection.toArray()

                    const availableDataElements = programStages.map(
                        programStage => {
                            ...
                            return {
                                ...
                                items: programStageDataElements.map(
                                    ({ dataElement }) => ({
...
const hasMissingProgram = compose(
    isNil,
    get('program')
)
