...
class AssignTrackedEntityTypeAttributes extends Component {
    ...
    componentDidMount() {
        this.context.d2.models.trackedEntityAttribute
            .list({
                level: 1,
                paging: false,
                fields: [
                    'id,displayName,valueType,unique,optionSet,mandatory,searchable,displayInList'
                ]
            })
    ...
    onEditAttribute = changedAttribute => {
        const newAssignedAttributes = this.state.assignedAttributes.map(
            attribute =>
    ...
    onMoveAttributes = newAttributesOrderIds => {
        const newAssignedAttributes = newAttributesOrderIds.map(
            attributeId =>
    ...
    getTableRows = () =>
        this.state.assignedAttributes.map(trackedEntityTypeAttribute => (
            <TETAttributeRow
    ...
    updateState = (assignedAttributes, assignedAttributesIds) => {
        assignedAttributesIds &&
            this.state.assignedAttributesStore.setState(assignedAttributesIds)
            ...
            <div>
                <div style={styles.groupEditor}>
                    <div style={styles.fieldname}>
                        {this.getTranslation('tracked_entity_type_attributes')}
                    </div>
                    <TextField
                ...
                <Table>
                    ...
                    >
                        <TableRow>
                            ...
                            <TableHeaderColumn>
                                {this.getTranslation('display_in_list')}
                            </TableHeaderColumn>
                            <TableHeaderColumn>
                                {this.getTranslation('mandatory')}
                            </TableHeaderColumn>
                            <TableHeaderColumn>
                                {this.getTranslation('searchable')}
                            </TableHeaderColumn>
                        </TableRow>
                    </TableHeader>
