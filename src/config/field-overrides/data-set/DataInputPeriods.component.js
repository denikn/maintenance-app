...
class DataInputPeriods extends React.Component {
    ...
    renderPeriods() {
        ...
        const changeDateProxy = ($k, dateField) =>
            this.changePeriodDate.bind(this, $k, dateField)

        const removeDateProxy = ($k, dateField) => () =>
            this.changePeriodDate($k, dateField, null, null)

        return (
            this.state.dataInputPeriods &&
            this.state.dataInputPeriods.map((dataInputPeriod, $k) => (
                <div key={$k}>
                    <div style={styles.periodRow}>
                        <div style={styles.periodColumn}>
                            <IconButton
                                style={styles.iconButton}
                                onClick={removePeriodProxy($k)}
                            >
                                <FontIcon
                                    className="material-icons"
                                    color="rgba(0,0,0,0.35)"
