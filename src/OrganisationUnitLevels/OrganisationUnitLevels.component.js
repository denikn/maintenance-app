...
function saveOrganisationUnitLevels(i18n) {
    actions.saveOrganisationUnitLevels().subscribe(
        () =>
            snackActions.show({
...
function OrganisationUnitLevels(props, context) {
    ...
    const fieldRows = props.fieldsForOrganisationUnitLevel.map(
        (fieldsForLevel, index) => {
            ...
            <Paper style={styles.paperWrap}>
                ...
                <FormButtons>
                    {canEdit ? (
                        <SaveButton
                            onClick={() =>
                                saveOrganisationUnitLevels(context.d2.i18n)
                            }
                            isValid={props.formStatus.every(v => v)}
