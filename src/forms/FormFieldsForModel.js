...
const fieldNamesToIgnoreOnDisplay = [
    'id',
    'publicAccess',
    'created',
    'lastUpdated',
...
class FormFieldsForModel {
    ...
    getFormFieldsForModel(model, fieldOverrides = {}, customFieldOrderName) {
        ...
        const onlyWritableProperties = modelValidation =>
            modelValidation.writable;
        const onlyPersistedProperties = modelValidation =>
            modelValidation.persisted;
        const toArrayOfFieldConfigurations = fieldName => {
            const modelValidationForField =
                model.modelDefinition.modelValidations[fieldName];
            // TODO: This is a horrible horrible hack that has to go... As soon as the API is fixed!
            const fieldConfig = Object.create(
                Object.assign(
